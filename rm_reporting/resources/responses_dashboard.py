...
def get_report_figures(survey_id, collection_exercise_id):
    ...
    logger.info(
        "Getting cases for collection exercise", survey_id=survey_id, collection_exercise_id=collection_exercise_id
    )
    case_result = case_controller.get_exercise_completion_stats(collection_exercise_id)
    if getattr(case_result[0], "Sample Size") == 0:
        logger.error(
            "No cases found for collection exercise", survey_id=survey_id, collection_exercise_id=collection_exercise_id
        )
    ...
    logger.info(
        "Getting business IDs for collection exercise",
        survey_id=survey_id,
    ...
    logger.info(
        "Getting enrolments for collection exercise", survey_id=survey_id, collection_exercise_id=collection_exercise_id
    )
